...
const HavaleYonetimContent = () => {
    ...
    useEffect(() => {
        const checkAdminAndFetchPosts = async () => {
            ...
            try {
                ...
                setIsAdmin(true);
                const response = await axios.get('http://localhost:8000/api/odeme/havaleler', {
    ...
    const handleDelete = async () => {
        const token = localStorage.getItem('authToken');
        try {
            await axios.delete(`http://localhost:8000/api/odeme/havale/${selectedHavaleId}`, {
                headers: {
    ...
    const handleUpdate = async (e) => {
        ...
        const token = localStorage.getItem('authToken');
        try {
            await axios.patch(`http://localhost:8000/api/odeme/havale/${selectedHavaleId}`, {
                ...selectedHavale,
                status: status
            }, {
                headers: {
