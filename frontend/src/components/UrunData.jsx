...
const UrunData = () => {
    ...
    const confirmDelete = async () => {
        if (productToDelete) {
            try {
                const token = localStorage.getItem('authToken');
                await axios.delete(`http://localhost:8000/deletePost/${productToDelete}`, {
                    headers: {
                        'Authorization': `Bearer ${token}`
                    }
                });
                ...
                <table className="min-w-full bg-white">
                    <thead className="bg-gray-50">
                        <tr>
                            ...
                            <th className="px-3 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Satışta mı?</th>
                    ...
                    <tbody className="divide-y divide-gray-200">
                        {currentProducts.length > 0 ? (currentProducts.map((product) => (
                            <tr key={product._id} className="hover:bg-gray-50 border p-5 cursor-pointer">
                                ...
                                <td className="px-4 py-2 whitespace-nowrap">
                                    <span className="px-1 inline-flex text-xs leading-4 font-semibold text-blue-800">
                                        {Array.isArray(product.size) ? (
                                            product.size.map((size, index) => (
                                                <div value={size} key={`existing-${index}`} className='relative p-0.5'>
                                                    <span className='bg-gray-600 rounded-md border text-white p-1'>{size}</span>
                                                </div>
                                            ))
                                        ) :  (
                                            <div>
                                ...
                                <td className="px-4 py-2 whitespace-nowrap text-sm text-gray-500">
                                    {product.selling === 'false' ? 'Hayır' : 'Evet'}
                                </td>
                            ...
                            <tr>
                                <td colSpan="10" className="text-center montserrat text-black py-8">Listelenecek ürün bulunamadı.</td>
                            </tr>
                        )}
                    </tbody>
                </table>
