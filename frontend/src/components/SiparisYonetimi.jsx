...
function SiparisYonetimi() {
    ...
    const handleApproveOrder = async (id) => {
        try {
            const response = await axios.patch(`http://localhost:8000/api/orders/${id}/approve`, {}, {
                headers: {
    ...
    const handleUpdateOrder = async (e) => {
        ...
        try {
            const response = await axios.patch(`http://localhost:8000/api/orders/${selectedOrder._id}`,
                selectedOrder,
                {
                    headers: {
                        "Authorization": `Bearer ${localStorage.getItem('authToken')}`,
                            ...
                            currentOrders.map(order => (
                                ...
                                ${order.status === 'pending' ? 'bg-yellow-100' :
                                        order.status === 'kargo' ? 'bg-blue-100' :
                                            order.status === 'success' ? 'bg-emerald-200' :
                                                order.status === 'iade' ? 'bg-yellow-400' :
                                                    order.status === 'red' ? 'bg-red-300' : 'bg-gray-100'} hover:bg-opacity-90 text-xs font-light font-sans cursor-pointer`}>
                                    ...
                                    <td className=" items-center justify-center py-4 px-4 gap-2 flex ">
                                        ...
                                        {
                                            order.status === '' || order.status === 'pending' ? <div>
