...
function UrunDetayContent() {
    ...
    useEffect(() => {
        const fetchProductDetail = async () => {
            try {
                const response = await axios.get(`http://localhost:8000/getDetail/${id}`);
    ...
    const handleAddToCart = async (product) => {
        ...
        try {
            ...
            const payload = token
                ? { productId: product._id, quantity: 1, userId: userId, size: selectedSize }
                : { productId: product._id, quantity: 1, sessionId: sessionId, size: selectedSize };

            if (!selectedSize) {
                alert('Beden Seçimi Zorunludur.')
            }

            console.log("SEÇİLEN BEDEN", selectedSize);
    ...
    const handleBuyNow = async (product) => {
        ...
        if (selectedSize) {
            navigate('/odeme', { state: { cartItems: [...cartItems, { product, quantity, size: selectedSize }] } });
        }
                ...
                {product && (
                    <>
                        <div className="lg:w-1/2 mb-4 lg:mb-0">
                            <div className="relative overflow-hidden h-[35rem] object-cover">
                                {product.imageUrls && product.imageUrls.length > 0 ? (
                                    <img
                                        src={`http://localhost:8000${product.imageUrls[currentImageIndex]}`}
                                        alt={product.name}
                        ...
                        <div className="lg:w-1/2 lg:pl-8 p-6 rounded-lg bg-gray-100">
                            ...
                            <p className={`text-gray-600 py-1 w-20 ${product.stock > 10 ? 'bg-emerald-500 rounded-md text-xs text-center text-white' : product.stock > 1 && product.stock < 10 ? 'bg-yellow-500 rounded-md text-xs text-center text-white' : 'bg-red-500 rounded-md text-xs text-center text-white'} font-sans mb-4`}>

                                {product.stock > 10 ? 'Stokta' : product.stock > 1 && product.stock < 10 ? 'Sınırlı Stok' : 'Tükendi'}
                            ...
                            <div className="flex flex-col items-start mt-4 mb-5">
                                ...
                                <select
                                    ...
                                    required
                                    className={`mt-4 px-3 py-2 text-xs border rounded-md focus:outline-none focus:ring-2 
            ${sizeError ? 'border-red-500 focus:ring-red-500' : 'border-gray-300 focus:ring-blue-500'}`}
                                >
                                    <option className='text-sm' value="" disabled>Beden Seçiniz</option>
